...
int32_t main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (a[ind].second == 'L') {
                ...
                if ((int) parity[1][p].size() == 0) {
                    ...
                    ans[temp[i]] = abs (parity[1][0][tempsize-1] - a[temp[i]].first);
                    ans[temp[i]] = abs (parity[1][0][tempsize-1] - a[temp[i]].first);
                    parity[1][0].pop_back();
                }
            } else {
        ...
        // now, see if the left and right ones collide
        for (int i = 0; i < parity[0][0].size(); i++) {
            if (parity[1][0].size() == 0) {
                break;
            }

        }
        for (int i = 0; i < parity[0][1].size(); i++) {
