...
void solve() {
    ...
    int ans = INF;
    for (int left = 1; left <= 2 * n; left *= 2) {
        auto r1 = lower_bound(all(a), left < n ? a[left] : INF) - a.begin();
        for (int mid = 1; mid <= 2 * n; mid *= 2) {
            int ind = (int) r1 + mid;
            auto r2 = lower_bound(all(a), ind < n ? a[ind] : INF) - a.begin();

            int x = (int) r1 - 0;
            int y = (int) r2 - (int) r1;
            int z = n - (int) r2;

            int cost = f(x) + f(y) + f(z);
            cout << x << " " << y << " " << z << " " << cost << "\n";
            ckmin(ans, cost);
        }
    }
